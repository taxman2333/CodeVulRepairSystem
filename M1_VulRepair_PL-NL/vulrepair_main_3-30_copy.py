...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    combined_scores = []
    cwe_stats = collections.defaultdict(lambda: {"success": 0, "total": 0, "exact_match": 0, "edit_distance": 0})
    ...
    for batch in bar:
        ...
        # Extract CWE type from ground truth (assuming it's embedded in the data)
        cwe_type = re.search(r"CWE-\d+", ground_truth)
        cwe_type = cwe_type.group(0) if cwe_type else "Unknown"
        cwe_stats[cwe_type]["total"] += 1
        ...
        # Update CWE stats
        if best_exact == 1.0:
            cwe_stats[cwe_type]["success"] += 1
        cwe_stats[cwe_type]["exact_match"] += best_exact
        cwe_stats[cwe_type]["edit_distance"] += best_edit
    ...
    # Prepare CSV data
    csv_data = []
    for idx, (cwe_type, stats) in enumerate(cwe_stats.items(), start=1):
        exact_match_rate = int((stats["exact_match"] / stats["total"]) * 100)
        edit_distance_rate = int((stats["edit_distance"] / stats["total"]) * 100)
        combined_rate = int(((0.6 * stats["exact_match"] + 0.4 * stats["edit_distance"]) / stats["total"]) * 100)
        csv_data.append({
            "序号": idx,
            "漏洞类型": cwe_type,
            "精准匹配准确率": f"{exact_match_rate}%",
            "精准匹配比例": f"{stats['success']}/{stats['total']}",
            "编辑距离准确率": f"{edit_distance_rate}%",
            "编辑距离比例": f"{stats['success']}/{stats['total']}",
            "综合准确率": f"{combined_rate}%"
        })
    
    # Sort by 综合准确率
    csv_data = sorted(csv_data, key=lambda x: int(x["综合准确率"].strip('%')), reverse=True)
    
    # Save to CSV
    output_dir = "./test_results"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    csv_file = os.path.join(output_dir, "cwe_statistics.csv")
    pd.DataFrame(csv_data).to_csv(csv_file, index=False, encoding="utf-8-sig")
    logger.info(f"CWE statistics saved to {csv_file}")
...
def setup_logging_with_command(args, mode="train"):
    ...
    # 配置日志格式
    logging.basicConfig(
        format='%(asctime)s - %(levelname)s - %(name)s -   %(message)s',  # 修复了这里的拼写错误
        datefmt='%m/%d/%Y %H:%M:%S',
        level=logging.INFO
    )
