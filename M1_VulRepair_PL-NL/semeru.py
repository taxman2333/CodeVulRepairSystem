...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, train_data=None, val_data=None, file_type="train"):
        if file_type == "train":
            sources = train_data["input"]
            labels = train_data["output"]
        elif file_type == "eval":
            sources = val_data["input"]
            labels = val_data["output"]
        elif file_type == "test":
            data = datasets.load_dataset("semeru/code-code-BugFixingMed", split="test")
            sources = data["input"]
            labels = data["output"]
        self.examples = []
    ...
    def __getitem__(self, i):       
        ...
        decoder_input_ids = self.examples[i].decoder_input_ids
        attention_mask = input_ids.ne(0)  # 生成 attention mask，非 0 的位置为 1
        return input_ids, attention_mask, label, decoder_input_ids
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    if args.n_gpu > 1:
        model = torch.nn.DataParallel(model)
    model.to(args.device)  # 确保模型在正确的设备上
...
def main():
    ...
    # 加载数据集
    if args.do_train:
        dataset = datasets.load_dataset("semeru/code-code-BugFixingMed")
        train_data = dataset["train"]
        val_data = dataset["validation"]
        train_dataset = TextDataset(tokenizer, args, train_data=train_data, file_type='train')
        eval_dataset = TextDataset(tokenizer, args, val_data=val_data, file_type='eval')
