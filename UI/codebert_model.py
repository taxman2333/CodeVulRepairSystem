...
class Beam(object):
    def __init__(self, size, sos, eos):
        ...
        self.scores = torch.zeros(size, dtype=torch.float, device='cuda')
        # The backpointers at each time-step.
        self.prevKs = []
        # The outputs at each time-step.
        self.nextYs = [torch.full((size,), 0, dtype=torch.long, device='cuda')]
        self.nextYs[0][0] = sos
    ...
    def getCurrentState(self):
        ...
        batch = self.nextYs[-1].clone().detach().view(-1, 1)
        return batch
